...
import type { GroupGoal, GroupGoalInput } from './types'

export function GroupGoalForm({
  initial,
  onSubmit,
  onCancel,
}: {
  initial?: GroupGoal
  onSubmit: (input: GroupGoalInput) => Promise<void>
  onCancel?: () => void
}) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4">
      {!initial && <h2 className="font-medium">Nueva meta del grupo</h2>}
      <input
        type="text"
        required
