...
import type { FundContributionInput } from './types'

export function CreateFundContributionForm({
  defaultDate,
  onSubmit,
  onCancel,
}: {
  defaultDate: string
  onSubmit: (input: FundContributionInput) => Promise<void>
  onCancel?: () => void
}) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-2 rounded-lg bg-slate-50 p-3">
      ...
      <div className="flex gap-2">
        ...
        >
          {submitting ? 'Guardando…' : 'Aportar al fondo'}
        </button>
        {onCancel && (
